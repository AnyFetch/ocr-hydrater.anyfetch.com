...
function isValid(document) {
  ...
  var invalidContentTypes = [
    "image/gif",
    "image/vnd.adobe.photoshop",
  ];

  var isExtensionValid = invalidExtensions.every(function(extension) {
    if(document.metadata && document.metadata.path && document.metadata.path.match(extension)) {
      return false;
    }
    return true;
  });

  var isContentTypeValid = invalidContentTypes.every(function(contentType) {
    if(document.metadata && document.metadata['content-type'] && document.metadata['content-type'].match(contentType)) {
      return false;
    }
    return true;
  });
...
module.exports = function(path, document, changes, finalCb) {
  ...
  async.waterfall([
    function convertDocument(cb) {
      ...
      shellExec('tesseract ' + path + " " + outPath, {timeout: 45000, killSignal: 'SIGKILL'}, function(err) {
        if(err) {
